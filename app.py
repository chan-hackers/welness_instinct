...
def chat_page():
    print(session['name'])
    if session['name']=='Madhumohan':
        chats = logics.get_chat_time_line(session['name'], 'naveen')
    elif session['name']=='naveen':
        chats = logics.get_chat_time_line(session['name'], 'Madhumohan')
    decrypted_chats = []
    for chat in chats:
...
def chat_upload():
    if request.method == 'POST':
        ...
        if session['name'] == "Madhumohan":
            ...
            logics.chat_time(session['name'], 'Madhumohan', encMessage)
...
def doc_city():
    ...
    return render_template('doc_city.html')
